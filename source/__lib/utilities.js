				...
				/* -------------------------------------------------- */
					...
					$(element).each(function() {
						...
						var	$windowScrollTop = $(window).scrollTop(),
							$elementPosition = self.position().top - $windowScrollTop;


						if ( $elementPosition <= 0 ) {

							//console.log($hash);
							console.log( $hashID + " is active.");


							var currentElement = ".link-" + $hash.replace(/\s|#/g, "");


							TweenMax.delayedCall( $delayInterval, function() {

																	console.log("Links updated!");

																	//$$(currentElement).addClass("is-active");
																	//$$(currentElement).parent().prev().addClass("is-active")
							...
							if( history.pushState ) {
								...
								window.location.hash.replace($hashID, "");
